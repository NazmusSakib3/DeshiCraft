...
function SellerProductTable({
  products,
  onDelete,
}: Readonly<{ products: Product[]; onDelete: (id: string) => void }>) {
              ...
              <td className="px-4 py-3">
                <div className="flex justify-end gap-2">
                  ...
                  <button
                    type="button"
                    onClick={() => {
                      if (confirm(`Delete "${p.title}"?`)) onDelete(p._id);
                    }}
  ...
  return (
    <div className="container-page py-10">
      ...
      {isLoading ? (
        ...
        <SellerProductTable products={products ?? []} onDelete={(id) => remove.mutate(id)} />
      )}
    </div>
  );
}
